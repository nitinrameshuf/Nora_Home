...
**The type scale is not CSS at all any more** — it is
`--force-device-scale-factor=1.25` on the wall's own Chromium
(`scripts/lib/provision-pi.sh` §7), and `html[data-surface="wall"]` is an empty
rule carrying the explanation. It *was* one line of CSS, three times over
(160%, 135%, then a clamped `vw`), and every version had the same flaw. Every
size in `nora-home.css` —
...
>
> **Changing it needs `./nora screens relaunch`**, added the same night: the
> launch scripts are generated by `provision-pi.sh`, so a deploy or a reboot
> will not pick up a new flag.
>
