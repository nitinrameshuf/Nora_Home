...
**The type scale is a setting now**, not a line of CSS and not a launch flag:
Settings → Screens, stored per screen, applied as CSS `zoom` on `<html>`
(`nora_home/ui/zoom.py`). It was one line of CSS three times over (160%, 135%,
a clamped `vw`) and a Chromium device scale factor once, and the reasoning for
each is in CLAUDE.md §4. Every size in `nora-home.css` —
...
>
> **Corrected again, and this is where it landed.** Asked "should that not
> scale automatically by display size?", which turned out to be the right
> question. A CSS pixel is *already* a reference pixel — the browser normalises
> for physical size via `devicePixelRatio` — and the one thing nothing can
> measure is viewing distance, which is exactly what `data-surface="wall"`
> carries. TV and signage platforms solve this one layer down (a 4K TV browser
> reports 1280 CSS px and lets the compositor upscale), and Chromium exposes the
> same lever. The wall now launches with `--force-device-scale-factor=1.25`.
>
> **Why every CSS version was wrong, including the `vw` one:** scaling the root
> font-size grows each `rem` while borders, shadows and radii stay
> 1-device-pixel hairlines, so the proportions come apart and it reads as
> *zoomed* even when the text size is right. That was the actual complaint, both
> times. A device scale factor scales everything together and the app lays out
> for the reported width instead of being magnified. 1.5 was tried first and
> overshot — it reports 1280, narrow enough that the topbar wrapped the profile
> icon under the greeting, and landed physically *larger* than the 135% it
> replaced. See CLAUDE.md §4.
>
> **And then it moved once more, to where it belongs.** A launch flag can only
> be changed over SSH, and screen size is a judgement only the person standing
> in front of the screen can make — so it is now a setting, applied as CSS
> `zoom`, which was measured against the flag and matches it exactly. See
> CLAUDE.md §4 and `nora_home/ui/zoom.py`.
>
> **`./nora screens` was reloading nothing** the whole time, which is what made
> the setting look broken: it sent `ctrl+shift+R` with xdotool, and Chromium in
> `--kiosk` ignores that. It now broadcasts a `refresh` over the displays bus.
>
