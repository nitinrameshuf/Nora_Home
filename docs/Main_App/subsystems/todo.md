...
**Sound is accepted and silently dropped**, not forwarded to `notify()`. No audio
channel exists anywhere in the platform's notification backends yet — that is
Story 38's job — so `Reminder.channels` can list `"sound"` today without it ever
producing an error, per CLAUDE.md's "failures degrade, never cascade."
